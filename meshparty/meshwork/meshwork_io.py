...
def save_meshwork_skeleton(filename, mw):
    ...
    with h5py.File(filename, 'a') as f:
        ...
        f.create_dataset("skeleton/mesh_to_skel_map", data=sk.mesh_to_skel_map, compression="gzip")
        if sk.radius is not None:
            f.create_dataset("skeleton/radius", data=sk.radius, compression="gzip")
        if sk.mesh_index is not None:
            f.create_dataset("skeleton/mesh_index", data=sk.mesh_index, compression="gzip")
...
def load_meshwork_skeleton(filename):
    with h5py.File(filename, 'r') as f:
        ...
        if 'mesh_index' in f['skeleton'].keys():
            ...
            mesh_index = None
        return Skeleton(verts, edges, root=root, radius=radius, mesh_to_skel_map=mesh_to_skel_map, mesh_index=mesh_index)
