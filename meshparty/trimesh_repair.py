...
try:
    ...
    logging.warning("Need to pip install caveclient to repair mesh with pychunkedgraph")
...
def np_shared_rows(A, B):
    ...
    good_edges = np.where(np.isin(A_flat, B_flat))[0]
    return good_edges
...
def find_close_edges_sym(vertices, labels, label_a, label_b):
    ...
    new_edges_a = find_close_edges(vertices, labels, label_a, label_b)
    new_edges_b = find_close_edges(vertices, labels, label_b, label_a)
...
def find_edges_to_link(
    mesh, vert_ind_a, vert_ind_b, distance_upper_bound=2500, verbose=False
):
    ...
    new_edges = find_close_edges_sym(
        mask_mesh.vertices, labels, labels[mask_inds[0]], labels[mask_inds[1]]
    )
    ...
    d_ais_to_all, pred = sparse.csgraph.dijkstra(
        graph,
        indices=mask_inds[0],
        unweighted=False,
        directed=False,
...
def merge_points_to_merge_indices(mesh, merge_event_points, close_map_distance=300):
    ...
    merge_edge_verts = merge_event_points.reshape(
        (
            merge_event_points.shape[0] * merge_event_points.shape[1],
    ...
    for k, (close_cc, arr) in enumerate(zip(closest_label[remap_merges], close_points)):
        ...
        if d.shape[0] != 0:
            ...
            close_inds[ind] = mesh.faces[
                arr[np.where(other_comp_points)[0][np.argmin(d)]], 0
            ]
...
def merge_log_edges(
    mesh, merge_log, base_resolution, close_map_distance=300, verbose=False
):
    ...
    if len(merge_event_points) > 0:
        merge_edge_inds = merge_points_to_merge_indices(
            mesh, merge_event_points, close_map_distance=close_map_distance
        )
    else:
        merge_edge_inds = []
...
def get_link_edges(
    ...
    if client is None:
        client = CAVEclient(datastack_name, server_address=server_address).chunkedgraph
