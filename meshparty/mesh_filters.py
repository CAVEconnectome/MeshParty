...
def filter_close_to_line(
    ...
    below_top = (
        mesh.vertices[:, axis] > line_end_pts[line_pt_ord[0], axis] - endcap_buffer
    )
    above_bot = (
        mesh.vertices[:, axis] < line_end_pts[line_pt_ord[1], axis] + endcap_buffer
    )
...
def filter_components_by_size(mesh, min_size=0, max_size=np.inf, map_to_unmasked=True):
    ...
    is_good = np.isin(labels, good_labels)
    if map_to_unmasked:
...
def filter_geodesic_distance(
    mesh, points, max_distance, max_valid_mapping=np.inf, map_to_unmasked=True
):
    """
    ...
    return np.invert(
        np.isinf(
            sparse.csgraph.dijkstra(
                mesh.csgraph, indices=inds, limit=max_distance, min_only=True
            )
        )
    )


def filter_two_point_distance(
    mesh, pts_foci, d_pad, indices=None, power=1, map_to_unmasked=True
):
    """
