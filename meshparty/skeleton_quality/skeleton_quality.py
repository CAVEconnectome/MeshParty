...
def skeleton_path_quality(
    sk,
    mesh,
    skind_to_mind_map=None,
    sk_norm=None,
    max_graph_dist=10000,
    p_ratio=None,
    bins=None,
    normalize_to_self=True,
    sliding=True,
    window=400,
    interval=50,
    return_path_info=False,
    filter_skind_map=True,
):
    """
    ...
    path_distances = matched_path_distances_mesh(
        sk_inds_list,
        mesh_inds_list,
        skind_to_mind_map,
        mesh,
        sk_norm=sk_norm,
        max_dist=max_graph_dist,
    )
    path_quality = []
    for pd in path_distances:
        if sliding:
            path_quality.append(
                pblast_score_sliding(
                    pd, p_ratio=p_ratio, bins=bins, normalize_to_self=normalize_to_self
                )
            )
        else:
            path_quality.append(
...
def compute_path_pairs(sk, mesh, skind_to_mind_map=None, return_indices=True):
    ...
    soma_boundary_plus_vec = (
        ((meshgraph * mesh_vec) > 0).astype(int) - mesh_vec
    ).astype(int)
    soma_boundary_inds = np.flatnonzero(
        np.logical_and((meshgraph * soma_boundary_plus_vec > 0), mesh_vec > 0)
    )

    ds, Ps, _ = csgraph.dijkstra(
        mesh.csgraph,
        return_predecessors=True,
        indices=soma_boundary_inds,
        min_only=True,
    )
    sk_paths = []
    ms_paths = []
...
def matched_path_distances_normalized(
    sk_paths, mesh_paths, sk_inds_list, sk_rs, r_min=50, from_path="skel"
):
    """
...
def matched_path_distances_mesh(
    sk_inds_list, mesh_inds_list, skind_to_mind_map, mesh, sk_norm=None, max_dist=10000
):
    """
    ...
    for skind_path, mind_path in tqdm(zip(sk_inds_list, mesh_inds_list)):
        ds = closest_graph_distance_paths(
            skind_to_mind_map[skind_path],
            mind_path,
            mesh,
            norm=sk_norm[skind_path],
            max_dist=max_dist,
        )
...
def closest_graph_distance_paths(
    path_A, path_B, mesh, norm=None, norm_min=50, max_dist=10000
):
...
def pblast_score(
    data, p_ratio=DEFAULT_P_RATIO, bins=DEFAULT_BINS_GRAPH, normalize_to_self=True
):
    """
...
def pblast_score_sliding(
    ...
    if len(data) < window:
        return pblast_score(
            data, p_ratio=p_ratio, bins=bins, normalize_to_self=normalize_to_self
        )
    else:
        worst_pscore = np.inf
        for ii in np.arange(0, len(data) - window + interval, interval):
            new_pscore = pblast_score(
                data[ii : ii + window],
                p_ratio=p_ratio,
                bins=bins,
