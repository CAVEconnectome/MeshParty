...
def compare_img_to_test_file(fname, back_val=255, close=15, pre_path=None):
    ...
    assert np.all(
        (np.mean(img_test_non_back, axis=0) - np.mean(img_tmpl_non_back, axis=0))
        < close
    )
    assert np.all(
        (np.std(img_test_non_back, axis=0) - np.std(img_tmpl_non_back, axis=0)) < close
    )
    return True
...
def eval_actor_360(
    actors, dir_name, tmp_path, camera=None, scale=2, nframes=30, make_image=False
):
    if make_image:
...
def test_full_cell_camera(full_cell_mesh, full_cell_soma_pt, tmp_path):
    ...
    eval_actor_image(
        [mesh_actor], "full_cell_orient_camera.png", tmp_path, camera=camera, scale=1
    )
    scale_bar_actor = trimesh_vtk.scale_bar_actor(
        full_cell_soma_pt - [15000, 0, 0], camera
    )
    eval_actor_image(
        [mesh_actor, scale_bar_actor],
...
def test_full_cell_with_links(full_cell_mesh, full_cell_merge_log, tmp_path):
    ...
    mesh_actor = trimesh_vtk.mesh_actor(
        full_cell_mesh, opacity=1.0, show_link_edges=True
    )
...
def test_point_cloud(full_cell_mesh, full_cell_synapses, full_cell_soma_pt, tmp_path):
    ...
    syn_actor = trimesh_vtk.point_cloud_actor(
        full_cell_synapses["positions"], size=sizes, color=(1, 0, 0), opacity=1
    )
    eval_actor_image(
        [mesh_actor, syn_actor],
    ...
    rand_colors = np.hstack(
        [x[:, np.newaxis], np.abs(x - 0.5)[:, np.newaxis], (1 - x)[:, np.newaxis]]
    )

    syn_actor = trimesh_vtk.point_cloud_actor(
        full_cell_synapses["positions"], size=500, color=rand_colors, opacity=1
    )
    eval_actor_image(
        [mesh_actor, syn_actor],
    ...
    syn_actor = trimesh_vtk.point_cloud_actor(
        full_cell_synapses["positions"], size=500, color=rand_colors_uint8, opacity=1
    )
    eval_actor_image(
        [mesh_actor, syn_actor],
    ...
    with pytest.raises(ValueError) as e:
        syn_actor = trimesh_vtk.point_cloud_actor(
            full_cell_synapses["positions"], size=300, color=np.random.rand(len(x), 2)
        )
